...
export default {
    data(){
        return {
            content: {
                msgTooltip: null,
                labelContent: 'Valor que você já possui investido',
                for: 'investimentoAtual',
                min: 0,
                max: 100000000,
                step: 1000,
                prefixo: 'R$',
    ...
    computed: {
        numberValue: {
            get() {
                return this.$store.state.inputs.baseInputs.investimentoAtual
            },
            set(value) {
                this.$store.commit('inputs/updateInvestimentoAtual', value)
            }
        }
    }
}
